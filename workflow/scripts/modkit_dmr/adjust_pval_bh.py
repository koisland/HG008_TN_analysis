...
def main():
    ...
    df = (
        ...
        .with_columns(map_pvalue_rank=pl.col("map_pvalue").rank())
        .with_columns(pl.col("map_pvalue_rank") / pl.col("map_pvalue_rank").min())
        .with_columns(bh_crit_value=(pl.col("map_pvalue_rank") / df.shape[0]) * FDR)
        .collect()
    )

    nrows_after_init_filter = df.shape[0]
    print(
        f"Removed {nrows - nrows_after_init_filter} after p-value filter {PVAL_FILTER}"
    )
    ...
    df = df.filter(pl.col("map_pvalue") < pl.col("bh_crit_value"))
    nrows_after_bh = df.shape[0]

    print(
        f"Removed {nrows_after_init_filter - nrows_after_bh} after BH with FDR of {FDR * 100}%"
    )
